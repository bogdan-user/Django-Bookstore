...
class ChatNotifyConsumer(AsyncHttpConsumer):
    ...
    async def handle(self, body):
        ...
        if is_employee:
            ...
            asyncio.get_event_loop().create_task(self.stream(self.is_streaming, self.no_poll))
    ...
    async def stream(self, is_streaming, no_poll):
    ...
    async def query_remote_server(self, order_id):
        async with aiohttp.ClientSession() as session:
            async with session.get(
                "https://pastebin.com/raw/1St61MXD"
            ) as resp:
